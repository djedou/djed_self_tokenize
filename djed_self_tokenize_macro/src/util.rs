...
pub fn generate_impl(ast: &syn::DeriveInput, to_tokens_impl: bool, to_custom_tokens_impl: bool, body: quote::Tokens) -> quote::Tokens {
    ...
    let to_tokens_impl_body = if to_tokens_impl {
        quote! {
            impl #impl_generics ::djed_self_tokenize_trait::ToTokens for #name #ty_generics #where_clause {
                fn to_tokens(&self, tokens: &mut ::djed_self_tokenize_trait::Tokens) {
    ...
    let to_custom_tokens_impl_body = if to_custom_tokens_impl {
        quote! {
            impl #impl_generics ::djed_self_tokenize_trait::ToCustomTokens for #name #ty_generics #where_clause {
                fn to_custom_tokens(&self, tokens: &mut ::djed_self_tokenize_trait::Tokens) {
                    #body
                }
            }
        }
